...
lease_duration = 5  # Lease time in seconds
    ...
    while True:
        ...
        with lease_table_lock:
            ...
            for client_address in expired_clients:
                ...
                print(f"Lease expired: Released IP {ip} for client {client_address}")
    ...
    if msg_type == 1:  # DHCP Discover
        ...
        with lease_table_lock:
            if client_address in lease_table and lease_table[client_address][0] == requested_ip:
                # Renew the lease
                lease_table[client_address] = (requested_ip, time.time() + lease_duration)
