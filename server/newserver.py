    ...
    while True:
        ...
        with lease_table_lock:
            ...
            for client_address, xid, mac_address in expired_clients:
                ...
                logging.info(f"Lease expired: Released IP {ip} for client {
                             client_address} (MAC: {mac_address}) (XID: {xid})")
                ...
                with discover_cache_lock:  # Ensure thread safety if discover_cache is shared across threads
                    for key in list(discover_cache.keys()):
                        ...
                        if discover_cache[key].get('mac_address') == mac_address:
                            ...
                            logging.info(f"Removed client {client_address} (MAC: {
...
def handle_client(message, client_address, server_socket):
    ...
    if msg_type == 1:  # DHCP Discover
        if client_address not in lease_table.keys():
        ...
        with lease_table_lock:
            if client_address in lease_table and lease_table[client_address][0] == requested_ip:
                ...
                lease_table[client_address] = (
                    ...
                    requested_lease, lease_table[client_address][2], lease_table[client_address][3]
                )

                # DHCP Ack with the requested lease duration
                ack_message = construct_dhcp_message(
                    xid, mac_address, msg_type=5, server_ip=server_ip, your_ip=requested_ip)
                # ack_message = struct.pack(
                #     "!I B 4s I", xid, 5, socket.inet_aton(
                #         requested_ip), requested_lease
                # )
                print(ack_message, client_tuple)
                server_socket.sendto(ack_message, client_tuple)
                logging.info(f"Assigned IP {requested_ip} to {client_tuple} (MAC: {
                             lease_table[client_address][3]}) with lease duration {requested_lease} seconds")
            else:
                ...
                logging.warning(f"Rejected IP request {requested_ip} from {
                                client_tuple} (MAC: {lease_table.get(client_address, ('',))[3]})")
        ...
        if client_address in lease_table:
            ...
            if lease_record[0] == declined_ip:
                ...
                lease_table[client_address] = lease_record
        ...
        logging.info(f"Received DHCP Release for IP {
                     released_ip} from {client_address}")
        if client_address in lease_table:
            lease_record = lease_table[client_address]
            lease_record = list(lease_record)
