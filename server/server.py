    ...
    if msg_type == 1:  # DHCP Discover
            ...
            if requested_ip and requested_ip in ip_pool:
                ...
                offer_message = struct.pack(
                    "!I B 4s 16s", xid, 2, socket.inet_aton(
                        requested_ip), bytes.fromhex(mac_address.replace(":", ""))
                )
        ...
        with lease_table_lock:
            if client_address in lease_table and lease_table[client_address][0] == requested_ip:
                ...
                logging.info(f"Assigned IP {requested_ip} to {client_address} (MAC: {
                ...
                logging.warning(f"Rejected IP request {requested_ip} from {
                                client_address} (MAC: {lease_table.get(client_address, ('',))[3]})")
